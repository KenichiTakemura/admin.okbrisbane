...
def add_user(user_name)
  ...
  if !user.present?
    ...
    if user.present?
      ...
      user = User.new(:email => "#{Common.unique_token}@okbrisbane.com", :password => Okvalue::DEFAULT_PASSWORD, :password_confirmation => Okvalue::DEFAULT_PASSWORD, :user_name => user_name, :is_special => false, :confirmed_at => Common.current_time)
      user.save
    end
  end
  user
